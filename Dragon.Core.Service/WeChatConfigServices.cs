...
{
    public class WeChatConfigServices : BaseService<WeChatConfig>, IWeChatConfigServices
    {
        readonly ILogger<WeChatConfigServices> _logger;
        readonly IBaseRepository<WeChatQR> _weChatQRRepository;
        readonly IBaseRepository<WeChatSub> _weChatSubRepository;
        readonly IBaseRepository<WeChatPushLog> _weChatPushLogReponsitory;
        public WeChatConfigServices(IBaseRepository<WeChatConfig> baseRepository,ILogger<WeChatConfigServices> logger, IBaseRepository<WeChatQR> weChatQRRepository, IBaseRepository<WeChatSub> weChatSubRepository, IBaseRepository<WeChatPushLog> weChatPushLogReponsitory) : base(baseRepository)
        {
            _logger = logger;
            _weChatQRRepository = weChatQRRepository;
            _weChatSubRepository = weChatSubRepository;
            _weChatPushLogReponsitory = weChatPushLogReponsitory;
        }

        public async Task<MessageModel<WeChatResponseUserInfo>> GetBindUserInfo(WeChatUserInfo info)
        {
            var bindUser = await _weChatSubRepository.Table.Where(t => t.SubFromPublicAccount == info.id && t.CompanyID == info.companyCode && info.userID.Equals(t.SubJobID) && t.IsUnBind == false).FirstOrDefaultAsync();
            if (bindUser == null) return MessageModel<WeChatResponseUserInfo>.Fail("用户不存在或者已经解绑!");
            var res = await GetToken(info.id);
            if (!res.success) return MessageModel<WeChatResponseUserInfo>.Fail(res.message);
        ...
        public async Task<MessageModel<WeChatApiDto>> GetMenu(string id)
        {
            ...
            var data = await WeChatHelper.GetMenu(res.result.access_token);
            if (data.errcode.Equals(0))
            {
        ...
        public async Task<MessageModel<WeChatResponseUserInfo>> GetQRBind(WeChatUserInfo info)
        {
            ...
            var data = await WeChatHelper.GetQRCode(res.result.access_token, pushJosn);
            ...
            data.id = info.userID;
            await _weChatQRRepository.InsertAsync(weChatQR);//this._dal.Db.Insertable<WeChatQR>(weChatQR).ExecuteCommandAsync();
            reData.usersData = data;
        ...
        public async Task<MessageModel<WeChatApiDto>> GetSubUser(string id, string openid)
        {
            var res = await GetToken(id);
            if (!res.success) return res;
            var data = await WeChatHelper.GetUserInfo(res.result.access_token, openid);
            if (data.errcode.Equals(0))
            {
                return MessageModel<WeChatApiDto>.Success("", data);
            }
        ...
        public async Task<MessageModel<WeChatApiDto>> GetSubUsers(string id)
        {
            var res = await GetToken(id);
            if (!res.success) return res;
            var data = await WeChatHelper.GetUsers(res.result.access_token);
            if (data.errcode.Equals(0))
            {
                data.users = new List<WeChatApiDto>();
                foreach (var openid in data.data.openid)
                {
                    data.users.Add(await WeChatHelper.GetUserInfo(res.result.access_token, openid));
                }
                return MessageModel<WeChatApiDto>.Success("", data);
            }
            else
        ...
        public async Task<MessageModel<WeChatApiDto>> GetTemplate(string id)
        {
            var res = await GetToken(id);
            if (!res.success) return res;
            var data = await WeChatHelper.GetTemplate(res.result.access_token);
            if (data.errcode.Equals(0))
            {
                return MessageModel<WeChatApiDto>.Success("", data);
            }
        ...
        {
            var config = await FindAsync(d=>d.publicAccount==publicAccount);
            if (config == null) 
                return MessageModel<WeChatApiDto>.Success($"公众号{publicAccount}未维护至系统");
            //还没过期,直接返回 
        ...
        {
            var bindUser = await _weChatSubRepository.Table.Where(t => t.SubFromPublicAccount == msg.info.id && t.CompanyID == msg.info.companyCode && t.IsUnBind == false && msg.info.userID.Contains(t.SubJobID)).SingleAsync();
            if (bindUser == null)
                return MessageModel<WeChatResponseUserInfo>.Fail("用户不存在或者已经解绑!");
            var res = await GetToken(msg?.info?.id);
            if (!res.success)
                return MessageModel<WeChatResponseUserInfo>.Fail(res.message);
            WeChatResponseUserInfo reData = new WeChatResponseUserInfo();
            ...
            try
            {
                ...
                var data = await WeChatHelper.SendCardMsg(res.result.access_token, pushJson);
                reData.usersData = data;
                try
                {
                    ...
                    await _weChatPushLogReponsitory.InsertAsync(pushLog);
                }
                catch (Exception ex)
                {
        ...
        public async Task<MessageModel<WeChatApiDto>> PushTxtMsg(WeChatPushTestDto msg)
        {
            ...
            else
            {
                ...
                {
                    //发送单个 
                    var usrs =_weChatSubRepository.Table.Where(t => t.SubFromPublicAccount.Equals(msg.selectWeChat) && t.CompanyID.Equals(msg.selectCompany) && t.SubJobID.Equals(msg.selectUser)).ToList();
                ...
                {
                    //发送所有
                    var usrs = _weChatSubRepository.Table.Where(t => t.SubFromPublicAccount.Equals(msg.selectWeChat) && t.CompanyID.Equals(msg.selectCompany)).ToList();
                    foreach (var item in usrs)
                    {
                        msg.selectUser = item.SubUserOpenID;
                        var info = await PushText(token, msg);
        ...
        public async Task<MessageModel<WeChatResponseUserInfo>> UnBind(WeChatUserInfo info)
        {
            ...
            await _weChatSubRepository.UpdateAsync(bindUser);//_dal.Db.Updateable<WeChatSub>(bindUser).UpdateColumns(t => new { t.IsUnBind, t.SubUserRefTime }).ExecuteCommandAsync();
        ...
        public async Task<MessageModel<WeChatApiDto>> UpdateMenu(WeChatApiDto menu)
        {
            ...
            if (!res.success) return res;
            var data = await WeChatHelper.SetMenu(res.result.access_token, JsonHelper.ObjToJson(menu.menu));
            if (data.errcode.Equals(0))
            {
        ...
        public async Task<string> Valid(WeChatValidDto validDto, string body)
        {
            ...
            try
            {
                ...
                var config = await FindAsync(d=>d.publicAccount== validDto.publicAccount);
                ...
                string encryption = MD5Util.Sha1(combineString).ToLower();
        ...
        {
            var ticket = await _weChatQRRepository.Table.SingleAsync(d=>d.QRticket== weChat.Ticket);
            ...
            var bindUser = await _weChatSubRepository.Table.Where(t => t.SubFromPublicAccount == ticket.QRpublicAccount && t.CompanyID == ticket.QRbindCompanyID && t.SubJobID == ticket.QRbindJobID).SingleAsync();
            bool isNewBind;
            if (bindUser == null)
            {
                isNewBind = true;
            ...
            try
            {
                ...
                await _weChatQRRepository.UpdateAsync(ticket);
                if (isNewBind)
                    await _weChatSubRepository.InsertAsync(bindUser);
                else
                    await _weChatSubRepository.UpdateAsync(bindUser);
